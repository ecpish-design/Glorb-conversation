# GLORB Conversation Mission Asset Inventory

This rebuild now uses the updated asset pack supplied as `GLORB CONVERSATION MISSION .zip`.

## Semantic UI asset mapping

- 1.png → glorb-full.png
- 2.png → glorb-portrait.png
- 3.png → glorb-side.png
- 4.png → glorb-side-portrait.png
- 5.png → story-start.png
- 6.png → story-join.png
- 7.png → story-end.png
- 8.png → combined-strip.png
- 9.png → start-strip.png
- 10.png → join-strip.png
- 11.png → end-strip.png
- 12.png → icon-greet.png
- 13.png → icon-open.png
- 14.png → icon-notice.png
- 15.png → icon-listen.png
- 16.png → icon-wait.png
- 17.png → icon-connect.png
- 18.png → icon-signal.png
- 19.png → icon-finish.png
- 20.png → icon-exit.png

The original uploaded numbered assets are also preserved in `assets/updated-asset-library/`.

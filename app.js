...
const state={
  ...
  start:{greet:null,open:null,notice:null,mobileStep:0},join:{timingDone:false,responseDone:false},end:{placed:[]},final:{start:false,join:false,end:false},refPage:0
};
const SESSION_KEY='glorbConversationMissionStateV4';
...
function joinTiming(){setTimeout(()=>{
  const yes=$('#joinYes'),no=$('#joinNo');
  yes.onclick=()=>{state.join.timingDone=true;saveSession();$('#timingFeedback').innerHTML='<div class="feedback good">✓ GOOD CHOICE. There is a pause. Glorb can join now.</div>';yes.disabled=true;no.disabled=true;$('#timingContinue').disabled=false};
  no.onclick=()=>{$('#timingFeedback').innerHTML='<div class="feedback">There is a pause now. Glorb can join.</div>';no.classList.add('tried');no.disabled=true};
  $('#timingContinue').onclick=()=>go(18);
  $('#replayTalk').onclick=()=>speakText('Maya says: The last round was so close. Milo says: I thought the ball was out. Then there is a pause.');
  $('#showTranscript').onclick=()=>{$('#helpContent').innerHTML='<p><strong>MAYA:</strong> The last round was so close.</p><p><strong>MILO:</strong> I thought the ball was out!</p><p><strong>PAUSE:</strong> There is a space to join.</p>';$('#helpDialog').showModal()};
},0);return shell(content(`<div class="timing-layout timing-simple"><div><div class="eyebrow orange">MISSION 02 // PRACTISE</div><h1 tabindex="-1">Help Glorb find a time to join</h1><p class="lead">Maya and Milo are talking. Help Glorb notice when there is a pause.</p><div class="conversation-card timing-conversation"><div class="bubble"><span class="speaker">MAYA</span>The last round was so close.</div><div class="bubble alt"><span class="speaker">MILO</span>I thought the ball was out!</div><div class="pause-box">PAUSE</div></div></div><div class="timing-question"><h2>Is this a good time for Glorb to join?</h2><div class="timing-answer-grid"><button id="joinYes" class="primary">YES — JOIN NOW</button><button id="joinNo" class="secondary">NOT YET</button></div><div id="timingFeedback"></div><div class="support-actions"><button id="replayTalk" class="ghost">Read conversation</button><button id="showTranscript" class="ghost">Show transcript</button></div><button id="timingContinue" class="primary" ${state.join.timingDone?'':'disabled'}>Continue</button></div></div>`),'paper')}
const joinChoices=[
...
function joinResponse(){setTimeout(()=>{$$('.join-choice').forEach(b=>b.onclick=()=>{if(b.dataset.type==='wrong'){b.classList.add('tried');b.disabled=true;b.innerHTML+=` <span class="status-tag">TRIED</span>`;$('#joinFeedback').innerHTML=`<div class="feedback">${b.dataset.feedback}</div>`}else{state.join.responseDone=true;saveSession();b.classList.add('correct');$$('.join-choice').forEach(x=>x.disabled=true);$('#joinFeedback').innerHTML=`<div class="feedback good">✓ GOOD CHOICE. ${b.dataset.feedback}</div>`;$('#completeJoin').disabled=false}});$('#completeJoin').onclick=()=>{state.completed=Math.max(state.completed,2);go(19)}},0);return shell(content(`<div class="assessment-shell"><div class="eyebrow orange">MISSION 02 // PRACTISE</div><h1 tabindex="-1">What could Glorb say now?</h1><div class="topic-banner">THEY ARE TALKING ABOUT: THE HANDBALL GAME</div><div class="join-choice-grid">${joinChoices.map(([t,type,fb])=>`<button class="choice join-choice" data-type="${type}" data-feedback="${esc(fb)}">${t}</button>`).join('')}</div><div class="assessment-footer"><div id="joinFeedback"></div><button id="completeJoin" class="primary" disabled>Finish JOIN mission</button></div></div>`),'paper')}
...
function screenHelp(){
  ...
  if(s===17)return '<p><strong>Help Glorb:</strong> Listen for a pause. When Maya and Milo stop talking, Glorb can join.</p>';
  if(s===18)return '<p><strong>Help Glorb:</strong> What are Maya and Milo talking about? Choose something about the same thing.</p>';
